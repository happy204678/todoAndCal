...
export default {
  ...
  watch: {
    topnum (val) {
      if (val.length < 1) {
        ...
        this.topnum = this.topnum.substring(0, 14)
      }
    },
    btmnum (val) {
      if (val.length < 1) {
  ...
  methods: {
    ...
    turnbtm (top) {
      this.btmnum = (Number(top) * 35.2739619).toString()
    },
    turntop (btm) {
      this.topnum = (Number(btm) / 35.2739619).toString()
    },
    goBack (val) {
